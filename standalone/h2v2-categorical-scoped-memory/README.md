...
списка (~33). Новый диалог ищет сессии **той же процедуры**. Рейтинг живёт на
...
- используемых procedure-id на пуле: 24–32
- доля `other` < 10%
- доля top-1 id < 20%
...
`copy-from-v1.sh` сам патчит `checks.py` (ending-enum v2 и сообщение про tag_key).
