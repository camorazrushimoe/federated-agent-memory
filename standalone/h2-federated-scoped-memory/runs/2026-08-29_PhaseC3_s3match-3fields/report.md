...
## 1. Run identity
- run dir: `2026-08-29_PhaseC3_s3match-3fields` (RUN-PROTOCOL §3)
...
**Primary read:** T.hit = 0.05 vs B1.hit = 0.0667 on the same n=60
...
- unique_served=5 top1_share=0.3287 top3_share=0.986 explore_fill=0.0 explore_promote=0 decay_fired=0
...
- `packet_tokens_p50` = 1088
- `packet_tokens_p95` = 1088
- `packet_tokens_max` = 1088
- `packet_tokens_per_query_mean` = 869.1
- `implied_agent_usd_per_1000` = None
- `serve_latency_p50` = 150.0
- `serve_latency_p95` = 158.0
- `token_method` = tag tokens from provider usage in raw/tag; packet tokens len(text)//4 fallback (EVAL-PLAN §4.6)
- `price_source` = None

## 8. Verdict
**NOT FIT — T.hit 0.05 <= B1.hit 0.0667 (ranker adds nothing over random similar session)**
