...
Strip rewiring (ROUND2-STRIP-PLAN §3/§3c): all pipeline imports now resolve
to canonical modules — `Config` (config.py), `compute_votes` (cluster.py),
`score_outcome` (eval.py, the ONE scoring path, C-EV5), `TFIDF`/`card_text`/
`now_iso` (common.py), `read_jsonl` (jsonio.py), `load_labels` (store.py).
Latent fix (recorded §3c): the hold-out slice is normalized to the INGESTED
shape (tenant→tenant_id 'abcd-', chat_id→dialogue_id 'd-', speaker→role)
before A1 touches it — the raw-pack holdout used to KeyError on tenant_id.
"""
...
from common import TFIDF, card_text, now_iso
from eval import score_outcome
...
def _strip_holdout_row(r: dict) -> dict:
    """Normalize a raw-pack hold-out row to the ingested (SPEC §3) shape
    (RUN-PROTOCOL §2.1) and strip label fields (C-L2). Fix recorded in
    ROUND2-STRIP-PLAN §3c."""
    if "dialogue_id" in r and "tenant_id" in r:
        base = r
    else:
        base = {
            "dialogue_id": "d-" + str(r["chat_id"]),
            "tenant_id": "abcd-" + str(r["tenant"]),
            "vertical": r.get("vertical", ""),
        }
    turns = []
    for t in r.get("turns", []):
        role = t.get("role") or t.get("speaker")
        if role in ("customer", "agent", "tool"):
            turns.append({"role": role, "text": t.get("text", "")})
    return {
        "dialogue_id": base["dialogue_id"],
...
def a1_serve_ceiling(pool: list[dict], holdout: list[dict],
    ...
    for h in holdout:
        ...
        model = TFIDF().fit([query] + texts)
        best = max(model.score(query, t) for t in texts)
        if best >= threshold:
            n_above += 1
...
def a2_oracle(holdout: list[dict], holdout_labels: dict) -> dict:
    """B2 oracle through the canonical ONE scoring function (C-EV5).

    A B2 packet IS the hold-out dialogue's true unlock_guideline (EVAL-PLAN
    §4.3) — identical to eval.py's B2 branch (`card_labels = [true_label]`).
    Must be >= 0.98, else the metric is broken."""
    n = len(holdout)
    hits = 0
    for h in holdout:
        tl = holdout_labels.get(h["dialogue_id"])
        outcome = score_outcome([tl] if tl is not None else [], tl)
        if outcome == "hit":
            hits += 1
    hit = round(hits / n, 6) if n else 0.0
    return {
        "value": hit,
        "n_holdout": n,
...
def a3_gate_binds(cards: list[dict], cfg, dialogues: dict) -> dict:
    ...
    for c in cards:
        ...
        if len(raw) >= 2:
            total_ge2 += 1
            votes, _mode = compute_votes(c, members, dialogues)
            if votes < cfg.get("K_INDEPENDENT"):
                blocked += 1
...
def a4_card_cosines(cards: list[dict], pool_labels: dict) -> dict:
    ...
    within: list[float] = []
    across: list[float] = []
    model = TFIDF().fit([t for _, t, _ in keyed])
    for i in range(len(keyed)):
        for j in range(i + 1, len(keyed)):
            s = model.score(keyed[i][1], keyed[j][1])
...
def main(argv=None) -> int:
    ...
    if args.cluster_threshold is not None:
        overrides["CLUSTER_THRESHOLD"] = args.cluster_threshold
    ...
    holdout = [_strip_holdout_row(r) for r in holdout_raw]

    fixtures = None
    if args.fixture_results:
        fixtures = json.loads(Path(args.fixture_results).read_text())

    dialogues_lookup = {d["dialogue_id"]: d for d in pool}
    audit = {
        "A1": a1_serve_ceiling(pool, holdout, cfg["MATCH_THRESHOLD"]),
        "A2": a2_oracle(holdout, holdout_labels),
        "A3": a3_gate_binds(cards, cfg, dialogues_lookup),
        "A4": a4_card_cosines(cards, pool_labels),
        "A5": a5_staleness(fixtures),
    }
    out = {"audited_at": now_iso(), "stage_gate": "S2",
           "items": audit,
