...
from eval import score_holdout, load_labels
from schema import (card_id_for, card_text, is_rejected, validate_card)
from store import read_jsonl, write_jsonl
from tfidf import TfidfModel
from scrub import EMAIL_RE, PHONE_RE, DIGITS_RE, TOKEN_RE

PII_PATTERNS = [EMAIL_RE, PHONE_RE, DIGITS_RE, TOKEN_RE]
...
class FixtureSuite:
    ...
    def extract(self, out: str = "cards.jsonl") -> dict:
        return _json(_run([str(HERE / "extract.py"), "--in", str(self.data / "dialogues.jsonl"),
                           ...
                           "--replay", "--clock-start", FIXTURE_NOW]
    ...
    def serve(self, live_file: str, cards: str = "cards.jsonl") -> dict:
        ...
        return _json(_run([str(HERE / "serve.py"), "--dialogue", str(live),
                           ...
                           "--packets-dir", str(self.data / "packets"),
                           "--clock-start", FIXTURE_NOW], self.work))
    ...
    def feedback(self, card_id: str | None, label: str, dialogue: str,
        ...
        cmd = [str(HERE / "feedback.py"), "--label", label, "--dialogue", dialogue,
               "--cards", str(self.data / cards),
               "--feedback-out", str(self.data / "feedback.jsonl"),
               "--clock-start", FIXTURE_NOW]
        if card_id:
            cmd += ["--card-id", card_id]
    ...
    def run_all(self) -> dict:
        ...
        _run([str(HERE / "extract.py"), "--in", str(self.data / "dialogues_gc.jsonl"),
              ...
              "--replay", "--clock-start", FIXTURE_NOW] + FixtureSuite._model_args(), self.work)
        ...
        for name in ("freshness_new_member", "freshness_quiet"):
            ...
            _run([str(HERE / "extract.py"), "--in", str(self.data / f"{name}.jsonl"),
                  ...
                  "--raw-dir", self.raw_dir, "--replay", "--clock-start", FIXTURE_NOW]
        ...
        _run([str(HERE / "extract.py"), "--in", str(rerun_dir / "dialogues.jsonl"),
              ...
              "--replay", "--clock-start", FIXTURE_NOW] + self._model_args(), self.work)
        ...
        r["reextract"] = _json(_run(
            [str(HERE / "extract.py"), "--in", str(self.data / "dialogues2.jsonl"),
             "--out", str(self.data / "cards2.jsonl"), "--raw-dir", self.raw_dir,
             "--replay", "--clock-start", FIXTURE_NOW] + FixtureSuite._model_args(),
            self.work))
        ...
        store2 = read_jsonl(self.data / "cards2.jsonl")
        r["sv5_orig"] = match_cards(d011, store2, self.cfg)
        r["sv5_perturbed"] = match_cards(perturbed, store2, self.cfg)
        ...
        for c in hole_cards:
            if c["card_id"] == card_id_for("d-001"):
                c["unlock"] = "none"
        ...
        for c in no_cards:
            if c["card_id"] == card_id_for("d-001"):
                c["unlock"] = "exchange blocked by tag"
            elif c.get("role") == "member" and c["card_id"] == card_id_for("d-002"):
                c["unlock"] = "refund issued directly"
...
def build_checks(ctx: Ctx) -> list[dict]:
    ...
    _expect(rows, "C-EX9", "extract", True,
            reextract.get("skipped_existing", 0) >= 1,
            f"re-extract skipped {reextract.get('skipped_existing')} clustered rows",
    ...
    rerun_identical = (rerun.get("ran") is False and rerun.get("merged", 0) == 0
    ...
    _expect(rows, "C-FB4", "feedback", True, ambiguous[0] == 2,
            ...
            "--card-id required when the packet held more than one card")
    ...
    _expect(rows, "C-EV5", "eval", True, _single_scoring_path(),
            "eval.py source scan: one score_holdout, arms routed via --arm",
...
def _check_cl4(cards: list[dict], dialogues: list[dict]) -> list[str]:
    ...
    for c in cards:
        ...
        votes, _ = compute_votes(c, members)
...
def _check_cl5(cards: list[dict], ctx: Ctx) -> list[str]:
    cards_by_id = {c["card_id"]: c for c in cards}
    clock = RunClock(ctx.clock_iso) if ctx.clock_iso else RunClock(cfgmod.utcnow_iso())
    viol = []
    for c in cards:
        if c.get("role") != "canonical":
            continue
...
def _check_cl6(cards: list[dict], ctx: Ctx, fixt: dict) -> list[str]:
    ...
    for c in cards:
        if c.get("role") != "canonical":
            continue
        members = [cards_by_id[m] for m in c.get("members", []) if m in cards_by_id]
        expect_lca = last_closed_at(c, members)
...
def _single_scoring_path() -> bool:
    ...
    defs = re.findall(r"def score_holdout", src)
...
def run_controls(ctx: Ctx, pool_raw_path: str, holdout_raw_path: str,
    ...
    packets_dir = str(ctx.run_dir / "packets")
    t_hit = ctx.metrics.get("primary", {}).get("unlock_hit_label", 0.0)
    clock_args = ["--clock-start", ctx.clock_iso] if ctx.clock_iso else []
    ...
    _run([str(HERE / "ingest.py"), "--in", nc1_src,
          ...
          "--agent-pool-size", "1", "--timeline", str(cfg["TIMELINE"]), "--t0", str(cfg["T0"])],
         one_work)
    _run([str(HERE / "extract.py"), "--in", str(one_work / "data" / "dialogues.jsonl"),
          "--out", str(one_work / "data" / "cards.jsonl"),
          "--raw-dir", str(ctx.run_dir / "raw" / "extract"), "--replay",
          "--model", "replay-fixture", "--base-url", "replay-fixture"] + clock_args,
         one_work)
    ...
    from match import match_cards
    served1 = sum(1 for d in holdout if match_cards(d, nc1_cards, cfg))
    ...
    served2 = sum(1 for d in holdout if match_cards(d, fake_cards, cfg))
    ...
    rows_shuf, met_shuf = score_holdout(
        arm="T", holdout=holdout, cards=cards, pool_dialogues=pool,
        pool_labels=shuffled_labels, holdout_labels=holdout_labels,
        packets_dir=packets_dir, cfg=cfg)
    hit_shuf = met_shuf["primary"]["unlock_hit_label"]
    prior = 0.0
    if holdout:
    ...
    rows_ts, _ = score_holdout(arm="T", holdout=holdout, cards=text_shuf,
                               pool_dialogues=pool, pool_labels=pool_labels,
                               holdout_labels=holdout_labels, packets_dir=packets_dir, cfg=cfg)
    ...
    def serve_rate_at(thr: float) -> float:
        ...
        return round(sum(1 for d in holdout if match_cards(d, cards, cfg2)) / len(holdout), 6) if holdout else 0.0
    res["NC4"] = {"hi_serve": serve_rate_at(0.99), "lo_serve": serve_rate_at(0.0)}

    # baselines through the same scoring path (C-EV3/4)
    _, b0 = score_holdout(arm="B0", holdout=holdout, cards=cards,
                          pool_dialogues=pool, pool_labels=pool_labels,
                          holdout_labels=holdout_labels, packets_dir=packets_dir, cfg=cfg)
    _, b1 = score_holdout(arm="B1", holdout=holdout, cards=cards,
                          pool_dialogues=pool, pool_labels=pool_labels,
                          holdout_labels=holdout_labels, packets_dir=packets_dir, cfg=cfg)
    _, b2 = score_holdout(arm="B2", holdout=holdout, cards=cards,
                          pool_dialogues=pool, pool_labels=pool_labels,
                          holdout_labels=holdout_labels, packets_dir=packets_dir, cfg=cfg)
    res["B0"] = b0
    res["B1"] = b1
