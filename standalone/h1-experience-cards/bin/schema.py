...
def card_id_for(dialogue_id: str) -> str:
    return "c-" + hashlib.sha256(dialogue_id.encode("utf-8")).hexdigest()[:12]
...
def card_text(card: dict) -> str:
    """The cluster/serve key: problem_shape + constraint + unlock (SPEC §6.3)."""
    return " ".join([card.get("problem_shape", ""),
                     card.get("constraint", ""),
                     card.get("unlock", "")])
