...
def main() -> int:
    ...
    r = subprocess.run([sys.executable, str(BIN / "extract.py"),
                        ...
                        "--raw-dir", str(raw), "--clock-start", "2026-08-28T12:00:00Z"],
                       capture_output=True, text=True, cwd=str(ROOT))
    if r.returncode != 0:
    ...
    model_x = TfidfModel([t for _, t, _, _ in keyed])
    w_x, a_x = [], []
    for i in range(len(keyed)):
        for j in range(i+1, len(keyed)):
            s = model_x.cosine(keyed[i][1], keyed[j][1])
