...
> Status: **no measured run yet** (D1 canonical `bin/` landed, PR #35; D2
> harness lands with this PR). Every line below is the committed structure;
> numbers are filled from the reference run when it lands (D8). No number in
> this file is ever hand-typed — each comes from a run dir (`metrics.json` /
> `cost.json`) or from `compare.py`.
