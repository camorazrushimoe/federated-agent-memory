# Run 2026-08-28_S2_B2

## 1. Identity

- arm: **B2** (baseline, LLM-free; round 4/4 — founder's final-round brief)
...
- C-EV1 HARD: `unlock_hit_label + wrong + abstain == 1.0` -> passed (hit 1.0 + wrong 0.0 + abstain 0.0 = 1.0)
...
- C-EV4 HARD: B2 oracle >= 0.98 -> passed (unlock_hit_label == 1.0)
...
```json
{
  "deterministic_half_wall_clock_s": 0.0,
  "extract": {
    "calls": 0,
    "completion_tokens": 0,
    "ms_p50": 0,
...
**Metric sanity gate (EVAL-PLAN §6.1 hard gate): PASSED** — oracle == 1.0 exactly. The scoring code is not broken.
